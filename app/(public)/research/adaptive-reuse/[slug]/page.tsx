...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const item = await getResearchItemBySlug("adaptive-reuse", slug);
  if (!item) return {};
  return {
    title: item.title,
...
export default async function AdaptiveReuseItemPage({ params }: Props) {
  const { slug } = await params;
  const item = await getResearchItemBySlug("adaptive-reuse", slug);
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      <nav className="flex items-center gap-1 text-sm text-stone mb-8 flex-wrap">
        ...
        <ChevronRight size={14} />
        <Link href="/research/adaptive-reuse" className="hover:text-terracotta transition-colors">Adaptive Reuse</Link>
        <ChevronRight size={14} />
