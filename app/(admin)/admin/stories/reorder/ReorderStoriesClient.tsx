...
function SortableStoryItem({ item }: { item: StoryItem }) {
...
export default function ReorderStoriesClient({ items }: { items: StoryItem[] }) {
  const { user } = useAdminAuth();
  const [orderedItems, setOrderedItems] = useState<StoryItem[]>(
          ...
          >
            <div className="flex flex-col gap-2">
              {orderedItems.map((item) => (
                <SortableStoryItem key={item.id} item={item} />
              ))}
            </div>
          </SortableContext>
        </DndContext>
