...
export type StoryCard = Pick<StoryItem, "id" | "title" | "image">;
...
export default function StoriesHighlights({ stories, featuredImages }: Props) {
  ...
  useScrollPositionRestore(scrollContainerRef, "storiesHighlightsScrollLeft");
        ...
        {stories.map((story, i) => {
          ...
          return (
            ...
            >
              ...
              >
                {/* ?story=<id> makes the stories page jump straight to this story */}
                <Link href={`/stories?story=${story.id}`} className="block">
                  <div className="relative w-full aspect-square overflow-hidden bg-walnut">
                    {story.image && (
                      <Image
                        src={story.image}
                        alt={story.title}
